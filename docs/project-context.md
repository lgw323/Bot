...
### Watch Together 30초 개설 유예 및 5초 퇴장 유예식 자동 소멸
- **개설 30초 유예 대기**: 세션이 최초 생성된 직후에는 참가자가 입장하기 전까지 즉시 소멸하는 것을 방지하기 위해 30초 동안의 소멸 유예 대기 시간을 보장합니다.
- **퇴장 5초 유예 대기**: 유저가 브라우저 창을 닫아 동시 시청방 커넥션 수가 0이 되었을 때, 네트워크 지연이나 새로고침으로 인한 방 파괴를 방지하기 위해 5초의 유예 대기 시간을 가집니다.
- **자동 파기 및 메시지 삭제**: 유예 시간이 지나 최종 파기 시점에 도달하면, 백그라운드 태스크에서 디스코드 채널의 초대 링크 임베드 메시지를 자동으로 삭제(ephemeral 메시지 제외)하여 채널 환경을 청결하게 유지하며, DB에서 방 세션 및 플레이리스트 데이터를 완전히 제거합니다.
