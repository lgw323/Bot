...
class MusicAgentCog(commands.Cog):
    def __init__(
        self,
        bot: commands.Bot,
        state_store: Optional[MusicStateStore] = None,
        session_restorer: Optional[MusicSessionRestorer] = None,
    ) -> None:
        self.bot: commands.Bot = bot
        self.state_store = state_store or MusicStateStore()
        self.session_restorer = (
            session_restorer or MusicSessionRestorer(bot)
        )
    ...
    async def cog_unload(self) -> None:
        ...
        await self.state_store.save(self.music_states)
        
        cleanup_tasks = [
            state.cleanup(leave=True, update_ui=False)
    ...
    async def on_ready(self) -> None:
        ...
        restored_states = await self.state_store.load_once()
        ...
        for guild in self.bot.guilds:
            ...
            if guild_id_str in restored_states:
                ...
                await self.session_restorer.restore(guild, state, data)
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState) -> None:
        if member.id == self.bot.user.id: # type: ignore
            ...
            if before.channel and not after.channel:
                ...
                if state and (state.current_song or state.queue):
                    # 비정상적 연결 종료(예: 1006)일 수 있으므로 즉시 정리하지 않고
                    # 재생 위치를 임시 저장한 후, 8초간 대기하며 discord.py의 자체 재연결을 기다립니다.
                    if state.current_song and state.voice_client and (state.voice_client.is_playing() or state.voice_client.is_paused()):
                        ...
                        logger.info(f"[{before.channel.guild.name}] 음성 끊김 감지: 재생 위치({state.seek_time}초) 저장 완료.")
                    ...
                    async def delayed_leave_check():
                        ...
                        current_state = self.music_states.get(guild_id)
                        if current_state and (not current_state.voice_client or not current_state.voice_client.is_connected()):
                            logger.info(f"[{before.channel.guild.name}] 음성 채널 재연결 실패 또는 강제 퇴장됨. 세션을 정리합니다.")
                            await self.leave_logic(guild_id)
                        elif current_state:
                            logger.info(f"[{before.channel.guild.name}] 음성 채널 재연결 성공. 세션 유지함.")

                    self.bot.loop.create_task(delayed_leave_check())
                else:
                    await self.leave_logic(guild_id) # type: ignore
            return
