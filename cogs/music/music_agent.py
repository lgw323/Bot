...
class MusicAgentCog(commands.Cog):
    ...
    async def cleanup_channel_messages(self, state: MusicState):
        ...
        try: await state.text_channel.purge(limit=100, check=lambda msg: msg.id != state.now_playing_message.id and not msg.pinned)
        except discord.HTTPException as e: logger.debug(f"[{state.guild.name}] 채팅 정리 중 오류: {e}")
