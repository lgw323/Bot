...
class WatchSessionControlView(discord.ui.View):
    """Private administrator control for one Watch Together session."""
    ...
    async def close_session_button(
        self,
        ...
        from cogs.watch_together.watch_server import close_watch_session
        ...
        button.label = "✅ 종료됨" if closed else "ℹ️ 이미 종료됨"
...
class LogAgentCog(commands.Cog, name="LogAgent"):
    ...
    async def send_watch_session_control(
        self,
        *,
        session_id: str,
        guild_id: int,
        ...
        """Send one private log-channel message with an administrator close button."""
        if LOG_CHANNEL_ID == 0:
            return
        ...
        if not isinstance(channel, discord.TextChannel):
            try:
                ...
                logging.warning(f"Watch Together 관리 채널 조회 실패: {e}")
                return
        ...
        embed = discord.Embed(
            title="🎬 Watch Together 세션 생성",
            description="필요하면 아래 버튼으로 세션을 즉시 종료할 수 있습니다.",
            color=discord.Color.blurple(),
            timestamp=discord.utils.utcnow(),
        )
        embed.add_field(name="세션", value=f"`{session_id}`", inline=False)
        embed.add_field(name="서버 ID", value=str(guild_id), inline=True)
        embed.add_field(
            name="생성자",
            value=f"<@{created_by}>",
            inline=True,
        ...
        await channel.send(
            embed=embed,
            view=WatchSessionControlView(session_id),
        )
