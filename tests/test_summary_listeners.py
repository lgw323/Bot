...
@pytest.mark.asyncio
async def test_prune_loop_follows_cog_lifecycle(mock_bot: MagicMock) -> None:
    with patch.dict(os.environ, {"GOOGLE_API_KEY": "test-key"}), \
         patch("cogs.summary.summary_listeners.initialize_gemini_client"), \
         patch("cogs.summary.summary_listeners.SUMMARY_CHANNEL_ID", 12345):
        cog = SummaryListenersCog(mock_bot)
        try:
            assert not cog.prune_old_messages.is_running()

            await cog.cog_load()
            assert cog.prune_old_messages.is_running()
        finally:
